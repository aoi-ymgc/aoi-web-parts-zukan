...
export default function Home() {
  const [query, setQuery] = useState(''); const [category, setCategory] = useState('すべて'); const [selectedSlug, setSelectedSlug] = useState('accordion'); const [modalOpen, setModalOpen] = useState(false); const [mobileOpen, setMobileOpen] = useState(false); const [favorites, setFavorites] = useState<string[]>([]);
  const selectedPart = parts.find((part) => part.slug === selectedSlug) ?? parts[0];
  const selectedCode = codeSamples[selectedPart.slug] ?? `<${selectedPart.slug}>${selectedPart.name}</${selectedPart.slug}>`;
  ...
  return <>
    ...
    <main id="top">
      ...
      <section id="preview" className="section detail-section"><div className="container"><p className="detail-breadcrumb">{selectedPart.category}　/　{selectedPart.name}</p><div className="detail-heading"><div><p className="eyebrow">INTERACTIVE PREVIEW</p><h2>{selectedPart.name}</h2><p>{selectedPart.description}</p></div><span className={`level level-${selectedPart.level}`}>{selectedPart.level}：{selectedPart.reason}</span></div><div className="interactive-panel"><div className="panel-label">触ってみる</div><div className="full-accordion"><PartPreview part={selectedPart} onModal={() => setModalOpen(true)} /></div></div><div className="detail-grid"><div><h3>どんなパーツ？</h3><p>{selectedPart.description}</p></div><div><h3>実装で気を付けること</h3><p>{selectedPart.reason}</p></div><div><h3>検索できる言葉</h3><p>{(searchAliases[selectedPart.slug] ?? []).slice(0, 4).join(' / ')}</p></div></div><div className="code-preview"><div><span>{selectedPart.name} のサンプルコード</span><button onClick={() => navigator.clipboard?.writeText(selectedCode)}>コピー</button></div><pre><code>{selectedCode}</code></pre></div></div></section>
